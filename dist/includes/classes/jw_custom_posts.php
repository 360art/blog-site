...
class JW_Post_Type
{
     ...
     */
    function __construct($name, $title = array(), $post_type_args = array())
    {
    ...
    function register_post_type()
    {
        ...
        if ( !($this->post_type_name == 'post' || $this->post_type_name == 'page') ) {
            $args = array(
                "label" => $this->post_type_title[0],
                'singular_name' => $this->post_type_title[1],
                "public" => true,
     ...
     */
    function add_taxonomy($taxonomy_name, $taxonomy_title, $options = array())
    {
        ...
        if (empty($taxonomy_title)) {
            array_push($taxonomy_title, ucwords($taxonomy_name) . 's', ucwords($taxonomy_name));
        }

        // At WordPress' init, register the taxonomy
        $this->init(
            function() use($taxonomy_name, $taxonomy_title, $post_type_name, $options)
            {
                ...
                $options = array_merge(
                    array(
                         "hierarchical" => false,
                         "label" => $taxonomy_title[0],
                         "singular_label" => $taxonomy_title[1],
                         "show_ui" => true,
                         "query_var" => true,
     ...
     */
    function add_meta_box($id, $title, $context = 'normal', $form_fields = array())
    {
        $post_type_name = $this->post_type_name;
        ...
        $this->admin_init(function() use($id, $title, $context, $form_fields, $post_type_name)
            {
                add_meta_box(
                    strtolower(str_replace(' ', '_', "meta")), // id
                    strtolower(str_replace(' ', '_', $title)), // title
                    function($post, $data)
